...
import { STRUCTURE_AABBS } from '../gridConfig'

/**
 * Streetlight placement, shared by the pole meshes (Structures) and the
 * wet-street light streaks (Atmosphere). Poles march both kerbs of the
 * avenue; anything hugging the camera rail too closely — or standing inside
 * a signature structure's no-fly box — is dropped, so the flight path gets
...
for (let z = 44; z >= -140; z -= 16) {
  for (const side of [-1, 1] as const) {
    const x = side * 10.6
    if (distanceToRail(x, z) < 2.2) continue
